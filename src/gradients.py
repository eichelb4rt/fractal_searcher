...
def gradient_approximation(f: Callable, x: npt.NDArray[np.float32], epsilon: float = 1e-2) -> npt.NDArray[np.float32]:
    ...
    for i in range(x.shape[0]):
        ...
        actual_distance = x_plus[i] - x_minus[i]
        gradient[i] = (f(x_plus) - f(x_minus)) / actual_distance
    return gradient


def gradient_descent(f: Callable, x_start: npt.NDArray[np.float32], learning_rate: float = 1e-2, num_steps: int = 100, gradient_approximation_epsilon: float = 1e-2) -> npt.NDArray[np.float32]:
    ...
    for _ in tqdm(range(num_steps), leave=False):
        ...
        # we need all entries of x in the unit interval
        x = np.clip(x, 0, 1)
        errors.append(f(x))
    return x, errors


def greedy_descent(f: Callable, x_start: npt.NDArray[np.float32], num_steps: int = 100, step_size: float = 1e-2) -> npt.NDArray[np.float32]:
...
def greedy_descent_2(f: Callable, x_start: npt.NDArray[np.float32], num_steps: int = 100, step_size: float = 1e-2) -> npt.NDArray[np.float32]:
    """Perform greedy gradient descent on a scalar function f starting at x_start."""
